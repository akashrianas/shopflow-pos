...
export function UsersAdmin() {
  ...
  async function create() {
    if (!form.email || !form.password || form.password.length < 6)
      return toast.error("Email and password (6+ chars) required");
    try {
    ...
    <div className="space-y-4">
      <Card className="glass p-4 space-y-3">
        ...
        <div className="grid gap-2 md:grid-cols-5">
          ...
          <Button onClick={create} className="gradient-primary text-white border-0">
            Create
          </Button>
        </div>
      </Card>
      ...
      <Card className="glass p-4">
        ...
        <div className="space-y-2">
          ...
          {!loading && users.length === 0 && (
            <div className="text-center py-6 text-muted-foreground text-sm">No users</div>
          )}
        </div>
      </Card>
    </div>
