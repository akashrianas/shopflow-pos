...
export const adminCreateUser = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { email: string; password: string; full_name: string; role: Role }) => data)
  .handler(async ({ context, data }) => {
    ...
    if (error || !created.user) throw new Response(error?.message ?? "Failed to create user", { status: 400 });
...
export const adminDeleteUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    await assertAdmin(context.supabase, context.userId);
    if (data.user_id === context.userId) throw new Response("Cannot delete yourself", { status: 400 });
    const sb = admin();
